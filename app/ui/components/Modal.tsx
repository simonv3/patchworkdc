const Modal: React.FC<{
  ...
  onConfirm?: () => void;
  isLoading?: boolean;
}> = ({ isOpen, onClose, children, onConfirm, confirmText, isLoading }) => {
  if (!isOpen) return null;

  return (
    <>
      <div
        className="fixed z-10 inset-0 bg-gray-500/75 transition-opacity"
        aria-hidden="true"
        onClick={onClose}
      ></div>
      <div
        ...
        className={`${
          isOpen ? "" : "hidden"
        } overflow-y-auto z-50 overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full`}
      >
          ...
          <div className="relative bg-white rounded-lg shadow-sm dark:bg-gray-700 dark:text-white">
            ...
            <div className="flex items-center p-4 md:p-5 border-t border-gray-200 rounded-b dark:border-gray-600">
              <button
                ...
                focus:ring-4 
                focus:outline-none 
                focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:focus:ring-blue-800"
              >
                {isLoading ? "Loading..." : (confirmText ?? "Let's go!")}
              </button>
              <button
                data-modal-hide="default-modal"
                type="button"
